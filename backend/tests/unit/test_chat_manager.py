...
class StubRecoveryService:
    async def find(self, session_id: str, name_query: str):
        return None, []


_STUB_RECOVERY = StubRecoveryService()
...
async def test_simple_intent_calls_llm(manager: ChatManagerService):
    ...
    response = await manager.handle(
        ChatRequest(session_id="s1", message="hola"), agent, _STUB_RECOVERY
    )
...
async def test_complex_intent_invokes_data_agent(manager: ChatManagerService):
    ...
    response = await manager.handle(
        ChatRequest(session_id="s1", message="muéstrame las ventas por mes"), agent, _STUB_RECOVERY
    )
...
async def test_complex_intent_truncated_mentions_limit(manager: ChatManagerService):
    ...
    response = await manager.handle(
        ChatRequest(session_id="s1", message="dame todas las filas"), agent, _STUB_RECOVERY
    )
...
async def test_complex_intent_empty_result_specific_message(manager: ChatManagerService):
    ...
    response = await manager.handle(
        ChatRequest(session_id="s1", message="dame las ventas del año pasado"), agent, _STUB_RECOVERY
    )
...
async def test_complex_intent_error_uses_error_message(manager: ChatManagerService):
    ...
    response = await manager.handle(
        ChatRequest(session_id="s1", message="dame las ventas"), agent, _STUB_RECOVERY
    )
...
async def test_history_accumulates_messages(manager: ChatManagerService):
    agent = StubDataAgent()
    await manager.handle(ChatRequest(session_id="s2", message="hola"), agent, _STUB_RECOVERY)
    await manager.handle(ChatRequest(session_id="s2", message="gracias"), agent, _STUB_RECOVERY)
...
async def test_history_isolated_between_sessions(manager: ChatManagerService):
    agent = StubDataAgent()
    await manager.handle(ChatRequest(session_id="session-a", message="hola"), agent, _STUB_RECOVERY)
    await manager.handle(ChatRequest(session_id="session-b", message="hola"), agent, _STUB_RECOVERY)
...
async def test_history_roles_are_correct(manager: ChatManagerService):
    agent = StubDataAgent()
    await manager.handle(ChatRequest(session_id="s3", message="hola"), agent, _STUB_RECOVERY)
...
async def test_llm_receives_full_history_as_context():
    ...
    agent = StubDataAgent()
    await manager.handle(ChatRequest(session_id="ctx", message="hola"), agent, _STUB_RECOVERY)
    await manager.handle(ChatRequest(session_id="ctx", message="gracias"), agent, _STUB_RECOVERY)
...
async def test_assistant_message_carries_extraction_and_trace(manager: ChatManagerService):
    ...
    await manager.handle(
        ChatRequest(session_id="trace-1", message="dame las ventas"), agent, _STUB_RECOVERY
    )
...
async def test_widget_generated_on_successful_extraction_with_rows(
    ...
    response = await manager.handle(
        ChatRequest(session_id="w1", message="ventas por región"), agent, _STUB_RECOVERY
    )
...
async def test_widget_not_generated_on_extraction_error(
    ...
    response = await manager.handle(
        ChatRequest(session_id="w2", message="drop table"), agent, _STUB_RECOVERY
    )
...
async def test_widget_not_generated_on_empty_result(
    ...
    agent = StubDataAgent(_success_extraction(row_count=0))
    response = await manager.handle(
        ChatRequest(session_id="w3", message="ventas del año 1800"), agent, _STUB_RECOVERY
    )
...
async def test_widget_not_generated_on_simple_intent(
    ...
    agent = StubDataAgent()
    await manager.handle(ChatRequest(session_id="w4", message="hola"), agent, _STUB_RECOVERY)
...
async def test_architect_receives_user_intent(
    ...
    await manager.handle(
        ChatRequest(session_id="w5", message="dame el top 5 de ventas"), agent, _STUB_RECOVERY
    )
...
async def test_architect_failure_does_not_break_the_response():
    ...
    agent = StubDataAgent(_success_extraction(row_count=3))
    response = await manager.handle(
        ChatRequest(session_id="w6", message="ventas"), agent, _STUB_RECOVERY
    )
...
async def test_widget_trace_attached_to_agent_trace():
    ...
    agent = StubDataAgent(_success_extraction(row_count=3))
    response = await manager.handle(
        ChatRequest(session_id="w7", message="ventas"), agent, _STUB_RECOVERY
    )
...
async def test_widget_preference_reuses_same_extraction_id(
    ...
    await manager.handle(
        ChatRequest(session_id="pref-1", message="muéstrame las ventas"), agent, _STUB_RECOVERY
    )
    ...
    pref_response = await manager.handle(
        ChatRequest(session_id="pref-1", message="prefiero scatter"), agent, _STUB_RECOVERY
    )
...
async def test_widget_preference_sets_preferred_type(
    ...
    await manager.handle(
        ChatRequest(session_id="pref-2", message="muéstrame las ventas"), agent, _STUB_RECOVERY
    )
    await manager.handle(
        ChatRequest(session_id="pref-2", message="prefiero scatter"), agent, _STUB_RECOVERY
    )
...
async def test_widget_preference_without_prior_extraction_falls_back_to_llm(
    ...
    agent = StubDataAgent()
    response = await manager.handle(
        ChatRequest(session_id="pref-3", message="prefiero scatter"), agent, _STUB_RECOVERY
    )
...
async def test_incompatible_preference_returns_explanation_and_keeps_session(
    ...
    first_response = await manager.handle(
        ChatRequest(session_id="pref-4", message="muéstrame las ventas"), agent, _STUB_RECOVERY
    )
    assert first_response.widget_spec is not None

    # Second turn: incompatible preference
    pref_response = await manager.handle(
        ChatRequest(session_id="pref-4", message="prefiero scatter"), agent, _STUB_RECOVERY
    )
    ...
    follow_up = await manager.handle(
        ChatRequest(session_id="pref-4", message="hola"), agent, _STUB_RECOVERY
    )
